...
const STACKS = ["node-rest", "tanstack-nestjs", "enterprise-reporting"] as const;
const STACK_ALIASES: Record<string, (typeof STACKS)[number]> = {
  ...
  // The reporting platform. `tanstack` stays bound to the NestJS stack above:
  // it was that stack's alias first, and silently moving it would change what
  // every existing `--stack tanstack` invocation generates.
...
${c.bold("STACKS")}
  ...
  ${c.bold("tanstack-nestjs")}
    The full application stack: a NestJS + Fastify + Kysely backend and a
    TanStack Start front end, generated as a complete standalone project.

  ${c.bold("enterprise-reporting")}
    TanStack Start + Kysely + PostgreSQL code shaped to drop into an
    enterprise_reporting_tanstack checkout: server functions using
    .inputValidator(), list/detail routes (shadcn/ui + TanStack Table), and a
    Kysely migration. Not a standalone app — it expects that repository's auth,
